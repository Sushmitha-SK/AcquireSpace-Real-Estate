...
const GoogleSignIn = () => {
    ...
    const handleGoogleSignIn = async () => {
        try {
            ...
            const resp: any = await googleSignIn(result.user.displayName, result.user.email, result.user.photoURL)
            dispatch(login(resp));
    ...
    return (
        <div className="flex items-center justify-center mt-7">
            <button type="submit"
                ...
                className="flex items-center bg-white dark:bg-gray-900 border border-gray-300
            rounded-lg shadow-md px-6 py-2 text-sm font-medium text-gray-800 dark:text-white hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
